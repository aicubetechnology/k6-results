...
import { API_BASE_URL, getHeaders, getMemorizePayload, checkStatus } from './utils-apikey.js';
...
export function handleSummary(data) {
  ...
  const scriptName = 'smoke-memorize-apikey';
...
function generateSummaryMarkdown(data, timestamp) {
  ...
  return `# Resultado do Teste de Carga Massiva: smoke-memorize-apikey
...
### Checks e Validações
* **Checks Passados:** ${metrics.checks.passes}
* **Checks Falhos:** ${metrics.checks.fails}
* **Taxa de Sucesso de Checks:** ${(metrics.checks.value * 100).toFixed(2)}%
* **Requisições com Falha:** ${(metrics.http_req_failed.value * 100).toFixed(2)}%

### Tempos de Resposta
* **Mínimo:** ${metrics.http_req_duration.min.toFixed(2)} ms
* **Médio:** ${metrics.http_req_duration.avg.toFixed(2)} ms
* **Mediana:** ${metrics.http_req_duration.med.toFixed(2)} ms
* **Máximo:** ${metrics.http_req_duration.max.toFixed(2)} ms
* **p(90):** ${metrics['http_req_duration']['p(90)'].toFixed(2)} ms
* **p(95):** ${metrics['http_req_duration']['p(95)'].toFixed(2)} ms
* **p(99):** ${metrics['http_req_duration']['p(99)'].toFixed(2)} ms

### Tempos de Processamento
* **Tempo Médio de Conexão:** ${metrics.http_req_connecting.avg.toFixed(2)} ms
* **Tempo Médio de TLS:** ${metrics.http_req_tls_handshaking.avg.toFixed(2)} ms
* **Tempo Médio de Envio:** ${metrics.http_req_sending.avg.toFixed(2)} ms
* **Tempo Médio de Espera:** ${metrics.http_req_waiting.avg.toFixed(2)} ms
* **Tempo Médio de Recebimento:** ${metrics.http_req_receiving.avg.toFixed(2)} ms

### Transferência de Dados
* **Dados Recebidos:** ${(metrics.data_received.count / (1024*1024)).toFixed(2)} MB (${metrics.data_received.rate.toFixed(2)} bytes/s)
* **Dados Enviados:** ${(metrics.data_sent.count / (1024*1024)).toFixed(2)} MB (${metrics.data_sent.rate.toFixed(2)} bytes/s)
* **Tamanho Médio de Resposta:** ${(metrics.data_received.count / metrics.http_reqs.count).toFixed(2)} bytes

### Recursos do Sistema
* **Duração Média de Iteração:** ${metrics.iteration_duration.avg.toFixed(2)} ms
* **Uso Máximo de VUs:** ${metrics.vus_max.value}
...
Este teste simulou 10 usuários simultâneos realizando 2 operações cada no endpoint \`/memorize\`, totalizando 20 requisições. Os resultados acima fornecem uma visão detalhada do desempenho do sistema sob esta carga simulada.

**Observação**: Este é um teste demonstrativo com parâmetros reduzidos. Para um teste de carga massiva real, seria necessário aumentar o número de VUs e iterações, além de executar em um ambiente que suporte maior volume de requisições simultâneas.
`;
}