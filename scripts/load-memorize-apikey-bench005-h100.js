...
import { API_BASE_URL, getHeaders, getMemorizePayload } from './utils-apikey-bench005-h100.js';
import { randomString } from './utils-apikey-bench005-h100.js';

// Array para armazenar requisições com falha
let failedRequests = [];
// Array para armazenar tempos de resposta
let responseTimes = [];
// Array para armazenar IDs de memórias criadas (para verificação posterior)
...
export const options = {
  ...
  thresholds: {
    ...
    http_req_duration: ['p(95)<1500'], // 95% das solicitações devem completar em menos de 1.5s
  },
};

export default function() {
  group('memorize_endpoint', function() {
    // Criar um payload único para esta requisição
    const payload = getMemorizePayload(`Teste de carga para memória - ${randomString(8)} - ${new Date().toISOString()}`);
    
    // Registrar o tempo de início
    const startTime = new Date().getTime();
    
    const res = http.post(`${API_BASE_URL}/memorize`, JSON.stringify(payload), { headers: getHeaders() });
    ...
    const success = check(res, {
      ...
      'has memory_id': (r) => {
        try {
          ...
          return body.memory_id !== undefined;
        } catch (e) {
          return false;
        }
      }
    ...
    if (success) {
      try {
        ...
        createdMemoryIds.push(body.memory_id);
      } catch (e) {
  ...
  sleep(Math.random() * 0.5 + 0.5); // Sleep entre 0.5 e 1 segundo
}

// Função para gerar os relatórios no final do teste
export function handleSummary(data) {
  // Diretório base para salvar os resultados
  const baseDir = 'results/bench-session-005/h100';
  
  // Tipo de teste e nome do script
  const testType = 'load';
  const scriptName = 'load-memorize-apikey';
  ...
  const summaryFiles = {
    ...
    // Gerar o arquivo created_memory_ids.json para referência
    [`${resultsPath}/created_memory_ids.json`]: JSON.stringify({
      note: "Este arquivo armazena os IDs das memórias criadas durante o teste.",
...
`;
  ...
  return `# Resultado do Teste de Carga: load-memorize-apikey (H100)

Data e hora: ${timestamp.toLocaleDateString()} ${timestamp.toLocaleTimeString()}

## Detalhes do Teste

* **Script:** load-memorize-apikey
...
* **Memórias Criadas:** ${createdMemoryIds.length}
...
Este teste de carga avalia o desempenho do endpoint \`/memorize\` do servidor H100 sob carga moderada. Os resultados mostram como o servidor responde ao aumento gradual de usuários virtuais, permitindo analisar a escalabilidade e estabilidade do sistema.
