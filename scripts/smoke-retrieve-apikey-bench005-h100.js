...
import { API_BASE_URL, getHeaders, getRetrievePayload, getMemorizePayload } from './utils-apikey-bench005-h100.js';
...
// Criar algumas memórias antes de começar o teste
export function setup() {
  // Criar 3 memórias para garantir que temos conteúdo para recuperar
  ...
  for (let i = 0; i < 3; i++) {
    ...
    if (res.status === 200) {
      console.log(`Memory created for retrieval test: ${JSON.parse(res.body).memory_id}`);
    } else {
...
export default function() {
  ...
  if (success) {
    try {
      ...
      console.log(`Retrieved ${body.matches.length} matches with source: ${body.source}`);
    } catch (e) {
...
`;
  ...
  return `# Resultado do Teste de Fumaça: smoke-retrieve-apikey (H100)
...
Este teste verifica a funcionalidade básica do endpoint \`/retrieve\`, que permite recuperar memórias relevantes com base em uma consulta.

Os resultados mostram o desempenho e a confiabilidade do servidor H100 ao processar requisições de recuperação de memórias.
`;
}