...
import { API_BASE_URL, getHeaders, TENANT_ID } from './utils-apikey-b200-high-quota.js';
...
export function handleSummary(data) {
  ...
  const scriptName = 'stress-get-tenant-stats-apikey-high-quota';
...
`;
  ...
  return `# Resultado do Teste de Stress: stress-get-tenant-stats-apikey (B200 - Alta Quota)
...
Este teste de stress simulou até 500 usuários virtuais fazendo consultas simultâneas ao endpoint \`/admin/tenant/${TENANT_ID}/stats\` no servidor B200, utilizando um tenant com quota alta (1.000.000 requisições diárias).

### Observações e Recomendações

1. **Capacidade de Escala**: A análise avalia a capacidade da API B200 de lidar com um alto volume de requisições simultâneas para consulta de estatísticas de tenant.

2. **Consistência das Respostas**: Analisa a consistência nos tempos de resposta sob alta carga.

3. **Comparação com H100**: Este teste foi executado com os mesmos parâmetros do teste realizado no servidor H100, permitindo uma comparação direta de desempenho.

4. **Próximos Passos**: Recomendações baseadas nos resultados observados.
`;
}