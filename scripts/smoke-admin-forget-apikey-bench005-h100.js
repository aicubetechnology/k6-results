...
import { API_BASE_URL, getHeaders, TENANT_ID, getForgetPayload } from './utils-apikey-bench005-h100.js';
  ...
  if (success) {
    try {
      ...
      check(body, {
        ...
        'has decayed_count field': (r) => r.decayed_count !== undefined,
        'has removed_count field': (r) => r.removed_count !== undefined,
      });
      
      console.log(`Forget operation successful: decayed ${body.decayed_count}, removed ${body.removed_count}`);
    } catch (e) {
...
export function handleSummary(data) {
  ...
  const scriptName = 'smoke-admin-forget-apikey';
...
`;
  ...
  return `# Resultado do Teste de Fumaça: smoke-admin-forget-apikey (H100)

Data e hora: ${timestamp.toLocaleDateString()} ${timestamp.toLocaleTimeString()}

## Detalhes do Teste

* **Script:** smoke-admin-forget-apikey
...
Este teste verifica a funcionalidade básica do endpoint \`/admin/forget\`, que permite aplicar o mecanismo de esquecimento (decay) às memórias de um tenant.

Os resultados mostram o desempenho e a confiabilidade do servidor H100 ao processar requisições de esquecimento.
`;
}