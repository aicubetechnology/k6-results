...
import { API_BASE_URL, getHeaders, TENANT_ID, TEST_AGENT_ID, getMemorizePayload } from './utils-apikey-bench005-b200.js';
...
export function setup() {
  // Criar uma memória para o teste
  const memorizeUrl = `${API_BASE_URL}/memorize`;
  const payload = getMemorizePayload(`Memória de teste para get-memory - ${new Date().toISOString()}`);
  ...
  if (res.status === 200) {
    try {
      ...
      console.log(`Created test memory with ID: ${createdMemoryId}`);
      return { memory_id: createdMemoryId };
    } catch (e) {
      console.error(`Error parsing memorize response: ${e.message}`);
      return { error: e.message };
    }
  } else {
    console.error(`Failed to create memory: ${res.status} - ${res.body}`);
    return { error: res.body };
  }
}

export default function(data) {
  // Se não temos ID de memória do setup, verificamos se já temos um armazenado
  let memoryId = data.memory_id || createdMemoryId;
  
  // Se ainda não temos um ID, criamos uma nova memória
  if (!memoryId) {
    const memorizeUrl = `${API_BASE_URL}/memorize`;
    const memorizePayload = getMemorizePayload(`Memória de teste para get-memory - ${new Date().toISOString()}`);
    ...
    if (memorizeRes.status === 200) {
      try {
        ...
        createdMemoryId = memoryId; // Salvar para próximas iterações
  ...
  const url = `${API_BASE_URL}/memory/${memoryId}`;
...
`;
  ...
  return `# Resultado do Teste de Fumaça: smoke-get-memory-apikey (B200)
...
Os resultados mostram o desempenho e a confiabilidade do servidor B200 ao processar requisições de obtenção de detalhes de memória.
`;
}