...
import { API_BASE_URL, getHeaders, TENANT_ID } from './utils-apikey-bench005-h100.js';
...
export default function() {
  ...
  if (success) {
    try {
      ...
      check(body, {
        'has total_memories field': (r) => r.total_memories !== undefined,
        'has avg_importance field': (r) => r.avg_importance !== undefined,
        'has agent_count field': (r) => r.agent_count !== undefined,
        'has agent_distribution field': (r) => r.agent_distribution !== undefined,
      });
      
      console.log(`Tenant stats retrieved: ${body.total_memories} memories, ${body.agent_count} agents`);
    } catch (e) {
...
export function handleSummary(data) {
  ...
  const scriptName = 'smoke-get-tenant-stats-apikey';
...
`;
  ...
  return `# Resultado do Teste de Fumaça: smoke-get-tenant-stats-apikey (H100)
...
Este teste verifica a funcionalidade básica do endpoint \`/admin/tenant/{tenant_id}/stats\`, que retorna estatísticas sobre um tenant específico.

Os resultados mostram o desempenho e a confiabilidade do servidor H100 ao processar requisições de obtenção de estatísticas de tenant.
`;
}