...
export default function() {
  ...
  if (success) {
    try {
      ...
      console.log(`Tenant stats retrieved successfully: ${TENANT_ID} has ${body.total_memories} memories`);
    } catch (e) {
...
`;
  ...
  return `# Resultado do Teste Smoke: smoke-get-tenant-stats-apikey (B200)
...
Este teste simulou 5 usuários simultâneos realizando 2 operações cada no endpoint \`/admin/tenant/${TENANT_ID}/stats\`, totalizando 10 requisições. Os resultados acima fornecem uma visão detalhada do desempenho do sistema sob esta carga simulada.
`;
}