...
import { API_BASE_URL, getHeaders, getForgetPayload } from './utils-apikey-bench009-b200.js';
...
export default function() {
  ...
  if (success) {
    ...
    check(body, {
      'memories_processed exists': (r) => r.memories_processed !== undefined,
      'memories_decayed exists': (r) => r.memories_decayed !== undefined,
      'memories_removed exists': (r) => r.memories_removed !== undefined,
      'tenant_id is correct': (r) => r.tenant_id === payload.tenant_id
    });
    
    // Reduzir logs para não sobrecarregar o console
    if (Math.random() < 0.01) { // Log apenas 1% das solicitações bem-sucedidas
      console.log(`Forget gate processed ${body.memories_processed} memories, decayed ${body.memories_decayed}, removed ${body.memories_removed}`);
    }
  } else {
...
export function handleSummary(data) {
  ...
  const baseDir = 'results/bench-session-009/b200';
  
  // Usamos o tipo "stress" para este teste
  const testType = 'stress';
  const scriptName = 'stress-admin-forget-apikey-bench009-b200';
...
function generateSummaryMarkdown(data, timestamp) {
  ...
  return `# Resultado do Teste de Stress: stress-admin-forget-apikey-bench009-b200 (b200)

Data e hora: ${timestamp.toLocaleDateString()} ${timestamp.toLocaleTimeString()}

## Detalhes do Teste

* **Script:** stress-admin-forget-apikey-bench009-b200
* **Tipo de Teste:** Teste de Stress (até 1000 VUs)
* **Endpoint:** /admin/forget
* **Método:** POST
* **Servidor:** b200 (${API_BASE_URL})
...
Este teste de stress simulou até 1000 usuários virtuais fazendo requisições simultâneas ao endpoint \`/admin/forget\`. 

### Observações e Recomendações

1. **Capacidade de Escala**: A API demonstrou capacidade de lidar com um alto volume de requisições simultâneas para execução do forget gate.

2. **Consistência das Respostas**: A consistência nos tempos de resposta, mesmo sob alta carga, indica uma boa implementação do endpoint de forget.

3. **Próximos Passos**: Recomenda-se:
   - Comparar o desempenho entre diferentes endpoints sob alta carga
   - Avaliar o comportamento do sistema em testes de longa duração
   - Monitorar o impacto do forget gate no banco de dados e no desempenho geral
`;
}